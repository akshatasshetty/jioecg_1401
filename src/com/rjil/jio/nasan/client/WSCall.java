...
public class WSCall {
	public static void main(String[] args) throws JSONException {
		...
		//l.putdevicedata("rf1332");
		//l.updateClinicInfo("rf1232", "RF", "Ganesh", "vashi", "navi mumbai", "988428613", "", "sdfs@fortis.com", "");
	...
	public JSONObject verify(String clinic_id, String passwoord)
		...
		try {
            ...
            while(true ){
                        s = br.readLine();
                        System.out.println("Gogia"+s);
                        if(s==null || s.length() == 0)
                                    break;
                        bl.append(s);
            }
            br.close();
            System.out.println("Ashu"+bl.toString());
            
            JSONParser par = new JSONParser();
            org.json.simple.JSONObject job = (org.json.simple.JSONObject) par.parse(bl.toString());
            
            String message = (String) job.get("message:");
            System.out.println("Blllll--->"+message);
	...
	public JSONObject getPassword(String clinic_id)
			throws JSONException {
		JSONObject data = new JSONObject();
		try {
	...
	public JSONObject changeDeviceStatus(String clinic_id, String device_id,
		...
		String line="";
		try {
			String path = "http://localhost:8080/jioecg_1401/rest/jioecg/activatedeactivate?clinic_id="+clinic_id+"&device_id="+ device_id+ "&device_status="+ device_status;
			HttpPost post = new HttpPost(path);
			DefaultHttpClient client = new DefaultHttpClient();
			HttpResponse httpresponse = client.execute(post);
			BufferedReader br = new BufferedReader(new InputStreamReader(
	...
	public JSONObject putclinicdata(String clinic_id) throws JSONException{
		...
		String line="";
		try{
			String query="http://localhost:8080/jioecg_1401/rest/jioecg/editclinic?clinic_id="+clinic_id;
	...
	/*public JSONObject putdevicedata(String clinic_id) {

		JSONObject data = new JSONObject();
		String line = "";
		try {
			String query = "http://localhost:8080/jioecg_1401/rest/jioecg/editdevice?clinic_id="
					+ clinic_id;
			HttpPost post = new HttpPost(query);
			DefaultHttpClient client = new DefaultHttpClient();
			HttpResponse httpresponse = client.execute(post);
	...
	public JSONObject updateClinicInfo(String clinic_id,String organisation_name, String organisation_head,
			String organisation_address, String city, String contact_no,
			String alternate_contact_no, String email_id,
