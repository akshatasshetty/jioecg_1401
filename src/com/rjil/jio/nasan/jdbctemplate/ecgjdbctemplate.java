...
public class ecgjdbctemplate {
		...
		try {
			...
			if (health.getClinic_id() != null) {
				if (cusertype == 0) {
					...
					sql11 = "select * from device_registration where device_id=?";
	...
	public int insertdata(String clinic_id, String patient_id,
			String patient_name, String age, String sex, String contact_number,
			String ecg_link, String jio_id, Timestamp ecgtest_date,
			Timestamp serverupload_time, String user_type, String test_type) {

		org.json.simple.JSONObject response = new org.json.simple.JSONObject();

		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm a");
		String time_stamp = sdf.format(ecgtest_date);
		...
		if (health2.getPatient_name() != null && health2.getClinic_id() != null) {
			...
			if (newecgtest_date.toString().equalsIgnoreCase(
				...
				String sql = "insert into jioecg_upload(clinic_id,patient_name,patient_id,sex,age,jio_id,contact_number,linkdownload,ecgtest_date,serverupload_time,user_type,time_stamp,test_type) values(?,?,?,?,?,?,?,?,?,?,?,?,?)";

				try {
					System.out.println("started1" + contact_number);
					count = jdbcTemplateObject
							.update(sql, clinic_id, patient_name, patient_id,
									sex, age, jio_id, contact_number, ecg_link,
									ecgtest_date, serverupload_time, user_type,
									time_stamp, test_type);
			...
			try {
				...
				count = jdbcTemplateObject.update(sql, clinic_id, patient_name,
						...
						ecgtest_date, serverupload_time, user_type, time_stamp,
						test_type);
				System.out.println(count);
				System.out.println("done");
			} catch (DataAccessException ex) {
	...
	public int register(String clinic_id, String password,
		...
		/*
		 * BigInteger phone_no = BigInteger.valueOf(contact_no); Integer
		 * alt_phone_no = Integer.valueOf(alternate_contact_no);
		 */
		...
		if (oldclinic_id.trim().equals("")) {
			...
			try {
				result1 = jdbcTemplateObject.update(sql12, clinic_id,
						organisation_name, device_id, device_name,
						device_status);
			} catch (Exception e) {// e.printStackTrace()
			}
	...
	public List<Map<String, Object>> clinicdata(String clinic_id,
			String user_type) throws JSONException {
	...
	public int getDeviceStatus(String clinic_id, String device_id,
			String device_status) {
		String sql = "";
		...
		int success = jdbcTemplateObject.update(sql, new Object[] { device_id,
				clinic_id });
		System.out.println(success);
		return success;
	}

	public String getecglink(String patient_id, String time_stamp) {

		verifylogin_pojo getlink = null;

		// String time_stamp1=URLEncoder.encode(time_stamp);
		// time_stamp="21-05-2014 12:38 PM";
		// System.out.println(patient_id+"		"+time_stamp+"		"+time_stamp1);
		// time_stamp="21-05-2014 12:38 PM";
		System.out.println(time_stamp);
		String query = "select * from jioecg_upload where patient_id= ? and time_stamp= ? ";
		System.out.println("printing query here " + query);
		try {
			getlink = jdbcTemplateObject.queryForObject(query, new Object[] {
					patient_id, time_stamp }, new verifyloginmapper());
		} catch (Exception e) {
			e.printStackTrace();
		}
		String ecglink = getlink.getLinkdownload();
		System.out.println("getting ecg link :   " + ecglink);
		// TODO Auto-generated method stub
		return ecglink;
	}

	public org.codehaus.jettison.json.JSONObject editclinicdata(String clinic_id)
			throws JSONException {

		org.codehaus.jettison.json.JSONObject json = new org.codehaus.jettison.json.JSONObject();
		String sql_edit = "Select * from clinic_registration where clinic_id= ?";
		try {
			clinic_registration getdata = jdbcTemplateObject.queryForObject(
					sql_edit, new Object[] { clinic_id },
					new clinic_registrationmapper());

			json.put("status", "success");
			json.put("clinic_id", getdata.getClinic_id());
			json.put("organisation_name", getdata.getOrganisation_name());
			json.put("organisation_head", getdata.getOrganisation_head());
			json.put("organisation_address", getdata.getOrganisation_address());
			json.put("organisation_city", getdata.getOrganisation_city());
			json.put("contact_number", getdata.getContact_number());
			json.put("alternative_contact_number",
					getdata.getAlternate_contact_number());
			json.put("email_id", getdata.getEmail_id());
			json.put("alternate_email_id", getdata.getAlternate_email_id());

		} catch (Exception e) {

			// e.printStackTrace();
			json.put("status", "Failed");
			json.put("Error", "Clinic id doesn't exist");
		}
		return json;
	}

	public int updateClinicInfo(String clinic_id, String organisation_name,
		...
		String sql = "", sql11 = "", sql22 = "", checkDeviceExist = "";
		int sql_success = 0, success = 0,device_exist1=0;
		int sql22_success=0;
		checkDeviceExist = "select count(*) as total from device_registration where clinic_id=?";
		device_registration device_exist = new device_registration();
		try{
		/*device_exist = jdbcTemplateObject.queryForObject(checkDeviceExist,
				new Object[] { clinic_id }, new device_registrationmapper());*/
			System.out.println(checkDeviceExist);
			device_exist1=jdbcTemplateObject.queryForObject(checkDeviceExist,
					new Object[] { clinic_id }, Integer.class);
			System.out.println(device_exist1);
		}
		catch(Exception e){}
		if (device_exist1!=0) {
			sql = "update clinic_registration set organisation_name=?,organisation_head=?,organisation_address=?,organisation_city=?,contact_number=?,alternate_contact_number=?,email_id=?,alternate_email_id=? where clinic_id=?";

			sql22 = "update device_registration set organisation_name=? where clinic_id=?";

			sql22_success = jdbcTemplateObject.update(sql22, new Object[] {
					organisation_name, clinic_id });

			sql_success = jdbcTemplateObject.update(sql, new Object[] {
					organisation_name, organisation_head, organisation_address,
					city, contact_no, alternate_contact_no, email_id,
					alternate_email_id, clinic_id });

			if (sql_success == 1 && sql22_success == 1)
				success = 1;
		} else {
			sql = "update clinic_registration set organisation_name=?,organisation_head=?,organisation_address=?,organisation_city=?,contact_number=?,alternate_contact_number=?,email_id=?,alternate_email_id=? where clinic_id=?";
			sql_success = jdbcTemplateObject.update(sql, new Object[] {
	...
	private int[] device_registrationmapper() {
		// TODO Auto-generated method stub
		return null;
	}

	public int updatedeviceInfo(String device_name, String device_id,
			String device_status, String clinic_id) {

		int response = 0, success = 0;

		String sql_updatedevice = "update device_registration set device_name= ? ,device_id= ? ,device_status= ? where clinic_id= ? ";

		response = jdbcTemplateObject.update(sql_updatedevice, new Object[] {
				device_name, device_id, device_status, clinic_id });
	...
	public int deletedevice(String clinic_id,String device_id) {

		int response = 0, success = 0;

		String sql_deletedevice = "delete from device_registration where clinic_id= ? and device_id= ? ";
		response = jdbcTemplateObject.update(sql_deletedevice,
				new Object[] { clinic_id,device_id });
		System.out.println("response " + response);
		if (response == 1)
			success = 1;
	...
	public org.codehaus.jettison.json.JSONObject getTestCount(String clinic_id) throws JSONException 
	{
		org.codehaus.jettison.json.JSONObject data=new org.codehaus.jettison.json.JSONObject();
		String query="SELECT count(*) as total FROM jioecg_upload where clinic_id = ?";
		int count=jdbcTemplateObject.queryForObject(query,new Object[] {clinic_id}, Integer.class);
